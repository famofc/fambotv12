...
case 'spamreactch': {
  if (!isOwner && !isReseller) {
    return m.reply('❌ This feature is for Premium users or the Owner only!');
  }

  if (!args[0] || !args[1]) {
    return m.reply(`❌ Incorrect format!\n\n✅ Example: .spamreactch https://whatsapp.com/channel/0029Vb2pMIt1NCrUCy9Q0f3C 😂`);
  }
  ...
  try {
    ...
    await m.reply(`✅ Successfully sent reaction ${emoji} to a message in channel *${channelInfo.name}*`);
  } catch (err) {
    console.error("SpamReactCh Error:", err);
    m.reply("❌ Failed to send reaction. Please make sure the link and emoji are valid.");
  }
}
break;
